...
모든 기술은 `staminaCost`만큼 기력을 사용하며 무료 기술도 `0`을 명시합니다. 기력이 부족한 입력은 기존 15틱 입력 버퍼 동안 대기합니다. 기력은 캐릭터의 `staminaRegen`만큼 매 60Hz 틱 회복되며 체력바 아래의 기력바에 표시됩니다. `kind: "guard"` 기술은 키를 누르는 동안 유지되고 `guardStaminaPerSecond`만큼 초당 기력을 소모하며, `damageReduction` 비율로 받는 피해를 줄입니다.
